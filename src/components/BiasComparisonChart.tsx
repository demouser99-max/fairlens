...
export function BiasComparisonChart({ before, after, metric, title }: Props) {
  const data = before.map((b, i) => ({
    group: b.group,
    Before: +(b[metric] * 100).toFixed(1),
    After: +(after[i][metric] * 100).toFixed(1),
  }));
    ...
    <div className="rounded-xl border border-border bg-card p-5 shadow-sm">
      ...
      <ResponsiveContainer width="100%" height={260}>
        <BarChart data={data} barGap={4} barSize={28}>
          ...
          <Bar dataKey="Before" fill="hsl(var(--chart-before))" radius={[4, 4, 0, 0]} />
          <Bar dataKey="After" fill="hsl(var(--chart-after))" radius={[4, 4, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
